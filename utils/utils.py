...
def group_gene_lens(ht, n, overwrite= False):
    if overwrite:
        ...
        sub = ht.select('gene_name', 'gene_id', 'transcript_id',
                    'chrom', 'start_pos', 'end_pos', 'length', 'idx', 'group_id')
...
def group_gene_interval(gtf, n, overwrite= False):
    ...
    gtf = gtf.annotate(new_idx=hl.if_else(gtf.n_previous_genes > 0,
                                          gtf.idx + n - gtf.n_previous_genes % n,
                                          gtf.idx))
    gtf = gtf.annotate(group_id=gtf.new_idx // n)
    gtf = gtf.select('gene_name', 'gene_id', 'transcript_id', 'chrom', 'start_pos', 'end_pos', 'length', 'idx', 'new_idx', 'group_id', 'n_previous_genes')

    gtf.checkpoint(f'{data_path}/gene_group_by_position_{n}_final.ht', _read_if_exists=not overwrite, overwrite = overwrite)
    group_ht = gtf.group_by('group_id', 'chrom').aggregate(group_start=hl.agg.min(gtf.start_pos),
                                                           group_end=hl.agg.max(gtf.end_pos),
                                                           genes=hl.agg.collect(gtf.gene_name))
    group_ht = group_ht.annotate(
        interval=hl.locus_interval(group_ht.chrom, group_ht.group_start, group_ht.group_end, reference_genome='GRCh38',
                                   includes_end=True, invalid_missing=True))

    group_ht.write(f'{data_path}/group_positions_{n}.ht', _read_if_exists=not overwrite, overwrite = overwrite)
...
def gt_to_gp(mt, location: str = 'GP'):
    return mt.annotate_entries(**{location: hl.or_missing(
        ...
        hl.map(lambda i: hl.cond(mt.LGT.unphased_diploid_gt_index() == i, 1.0, 0.0),
